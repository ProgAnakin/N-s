...
3. **SQL Editor**: run each file in `supabase/migrations/` **in order**.
   ...
   | `0014_what_is_yours.sql` | Private notes follow their author, not the space; `delete_my_data()` |
   ...
   **In order matters more than it looks.** The SQL Editor runs a whole file
   ...
   Five files check their prerequisites before doing anything and stop with
   ...
   need `0004`, and `0014` needs `0013`; `0012` needs every table it freezes to exist, and refuses
